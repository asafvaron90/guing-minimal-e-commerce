    ...
    MuiButton: {
      styleOverrides: {
        root: {
          ...
          // Universal interaction feedback that works on ANY fill (gradient, image, or solid). MUI's
          // default hover animates background-color, which is invisible behind an opaque gradient fill
          // — so we apply a filter here, once, for consistent hover/press across every button. Ripple,
          // focus ring, keyboard and disabled semantics come natively from MuiButton itself.
