...
import { FIGMA_IMAGE_CONTAINER_IMAGE_14_SVG } from './DealCard.assets';


export type LikeButtonSize = "small" | "large";
...
export type LikeButtonState = "unliked" | "liked";

export type DealCardSale = "yes" | "no";

export type DealCardFormat = "vertical" | "horizontal";

export interface DealCardProps {
  className?: string;
  style?: React.CSSProperties;
  likeButton?: LikeButtonConfig;
  sale?: DealCardSale;
  format?: DealCardFormat;
  oldPrice?: string;
  subtitle?: string;
  price?: string;
  productName?: string;
...
export const DealCard = ({ className, style, likeButton, sale, format, oldPrice, subtitle, price, productName, category, liked, onClick, onLikeToggle }: DealCardProps) => (
  <MuiBox data-figma-component data-fl-id="61:306" onClick={() => onClick?.()} className={className} style={{ position: 'relative', overflow: 'visible', width: sale == null && format == null ? '335px' : ((sale ?? "yes") === "yes" && (format ?? "horizontal") === "vertical" ? "160px" : ((sale ?? "yes") === "yes" && (format ?? "horizontal") === "horizontal" ? "335px" : ((sale ?? "yes") === "no" && (format ?? "horizontal") === "horizontal" ? "335px" : ((sale ?? "yes") === "no" && (format ?? "horizontal") === "vertical" ? "160px" : ('335px'))))), height: sale == null && format == null ? '170px' : ((sale ?? "yes") === "yes" && (format ?? "horizontal") === "vertical" ? "226px" : ((sale ?? "yes") === "yes" && (format ?? "horizontal") === "horizontal" ? "170px" : ((sale ?? "yes") === "no" && (format ?? "horizontal") === "horizontal" ? "170px" : ((sale ?? "yes") === "no" && (format ?? "horizontal") === "vertical" ? "226px" : ('170px'))))), display: 'flex', flexDirection: 'row', direction: 'ltr', justifyContent: 'flex-start', alignItems: 'center', gap: 'var(--gap-space-s, 8px)', boxSizing: 'border-box', background: 'var(--color-background-secondary, #f5f5f5)', borderRadius: sale == null && format == null ? 'var(--radius-radius-2xl, 24px)' : ((sale ?? "yes") === "yes" && (format ?? "horizontal") === "vertical" ? "0px" : ((sale ?? "yes") === "yes" && (format ?? "horizontal") === "horizontal" ? "24px" : ((sale ?? "yes") === "no" && (format ?? "horizontal") === "horizontal" ? "24px" : ((sale ?? "yes") === "no" && (format ?? "horizontal") === "vertical" ? "0px" : ('var(--radius-radius-2xl, 24px)'))))), ...style }}>
    <MuiBox data-fl-id="I61:306;49:12086" style={{ position: 'relative', width: '160px', flexShrink: 0, height: '170px', boxSizing: 'border-box', overflow: 'hidden', background: 'var(--color-background-secondary, #f5f5f5)', borderRadius: 'var(--radius-radius-2xl, 24px)' }}>
    <MuiBox data-fl-id="I61:306;49:12086;2:65" style={{ position: 'absolute', left: '-9px', top: '38px', width: '178px', height: '134px', boxSizing: 'border-box', lineHeight: 0, fontSize: 0 }} dangerouslySetInnerHTML={{ __html: FIGMA_IMAGE_CONTAINER_IMAGE_14_SVG }} />
  </MuiBox>
    <LikeButton size={likeButton?.size} state={likeButton?.state ?? (liked == null ? undefined : liked ? "liked" : "unliked")} changeIcon={likeButton?.changeIcon} icon={likeButton?.icon} unlikedIcon={likeButton?.unlikedIcon} likedIcon={likeButton?.likedIcon} onClick={likeButton?.onClick} onLikeToggle={onLikeToggle}/>
