...
export const fontVars = Object.fromEntries(
  (Object.keys(designTokens.typography) as TypographyKey[]).map((key) => {
    ...
    return [key, {
      ...
      style:         `var(${prefix}-style)`,
      lineHeight:    `var(${prefix}-line-height)`,
      letterSpacing: `var(${prefix}-letter-spacing)`,
      textTransform: `var(${prefix}-text-transform)`,
    }];
  })
) as { [K in TypographyKey]: { family: string; size: string; weight: string; style: string; lineHeight: string; letterSpacing: string; textTransform: string } };
...
type ShadowKey = keyof typeof designTokens.shadows;
/** All named Figma shadow tokens as CSS-variable references. */
export const shadowTokenVars = Object.fromEntries(
  (Object.keys(designTokens.shadows) as ShadowKey[]).map((key) => [
    key,
    `var(${toCssVar(key, '--shadow')})`,
  ])
) as { [K in ShadowKey]: string };
...
export const vars = {
  ...
  /** Semantic shadow CSS-variable strings (small/medium/large/inner). */
  shadow: shadowVars,
  /** All named Figma shadow token CSS-variable strings. */
  shadowToken: shadowTokenVars,
} as const;
